...
|| [**0.0|READ ME**](#h_0.0_readme) || [1.0|EXTRACT](#h_0.0_readme) || [2.0|TRANSFORM](#h_2.0_transform) || [3.0|LOAD and ANALYSE](#h_3.0_analyse) || <br>
...
#![Workflow process](.Images/CASA_GIS_diplomacy_detailed.png)
...
#lowy_data <- head(lowy_data,5) #FOR SAMPLE WORK
```
...
if(use_local_files) {
  ...
  #Save .csv file
  write.csv(lowy_data_geo_master,"./Data/GoogleAPI/lowy_data_geo_master.csv", row.names = FALSE)
}
...
f_exact_city_postings <- function(p_city_filter,p_df,p_colour = "red"){
  ...
  exact_map <- mapview(
      ...
      sub_city_geo_sf,
      legend = FALSE,
      layer.name = "Exact embassy locations",
  ...
  mapshot(exact_map, url = paste("./Data/MapFiles/",p_city_filter,"map.html",sep="_"))
...
```{r}
###run function
f_out <- f_exact_city_postings(p_city_filter = "Algiers",p_df = geo_lowy_no_NA)
f_out$map
...
f_exact_city_cluster <- function(p_city_filter,p_df,p_eps,p_k = 4,p_outlier_remove = FALSE){
  ...
  #add 2 to cluster to avoid 0 values as leaflet doesn't like to colour by 0 (zero) values and colour '1' is black
  db_coordinates_keep$cluster <- db$cluster+2
  ...
  f_out_cluster$df #[RETURN]
   ...
   mapshot(f_out_cluster_map, url = paste("./Data/MapFiles/",p_city_filter,"cluster_map.html",sep="_"))
...
#============== function end
#f_exact_city_cluster(p_city_filter = "Tehran",p_df = geo_lowy_no_NA,p_eps = 0.015,p_k =4,p_outlier_remove = TRUE)
#f_exact_city_cluster(p_city_filter = "London",p_df = geo_lowy_no_NA,p_eps = 0.005,p_k =4,p_outlier_remove = TRUE)

#f_exact_city_cluster(p_city_filter = "Manila",p_df = geo_lowy_no_NA,p_eps = 0.003,p_k =4,p_outlier_remove = TRUE)
#f_exact_city_cluster(p_city_filter = "Nairobi",p_df = geo_lowy_no_NA,p_eps = 0.010,p_k =4,p_outlier_remove = TRUE)
#f_exact_city_cluster(p_city_filter = "Singapore",p_df = geo_lowy_no_NA,p_eps = 0.005,p_k =4,p_outlier_remove = TRUE)

sample_anlaysis <- f_exact_city_cluster(p_city_filter = "Algiers",p_df = geo_lowy_no_NA,p_eps = 0.002,p_k =4,p_outlier_remove = TRUE)
sample_anlaysis$df_sub
```
...
|| [0.0|READ ME](#h_0.0_readme) || [1.0|EXTRACT](#h_1.0_extract) || [2.0|TRANSFORM](#h_2.0_transform) || [**3.0|LOAD and ANALYSE**](#h_3.0_analyse) || [**4.0|ACLED data]() || <br>
...
For details on how to use ACLED API please see documentation [here](){target="_blank"}
...
after this, filters and criteria can be concatenated useing `&`
...
f_acled_merge <- function(p_city_filter,p_df,p_eps,p_k,p_outlier_remove = TRUE,p_acled_year = FALSE,p_acled_city = TRUE){
  ...
  mapshot(f_acled_map, url = paste("./Data/MapFiles/",p_city_filter,"ACLED_map.html",sep="_"))
...
f_acled_results <- f_acled_merge(
  #standard parameters
  p_city_filter = "Manila",p_df = geo_lowy_no_NA,p_eps = 0.015,p_k =4,p_outlier_remove = TRUE,
  #acled parameters
  p_acled_year = 2019,
  p_acled_city = FALSE)
